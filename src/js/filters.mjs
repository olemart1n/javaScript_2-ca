...
export const searchForTags = () => {
    ...
    fetchCall(postswithac, getWithJwt).then((data) => {
        ...
        postContainer.insertAdjacentHTML("afterbegin", `<p>${filtered.length} posts with tags</p>`);
        //   commentList();
        //   likeClick();
    });
};
...
export const searchForTitle = () => {
    postContainer.textContent = "";
    fetchCall(postswithac, getWithJwt).then((data) => {
        const filtered = data.filter((data) => data.title.startsWith(searchInput.value));
        filtered.forEach((element) => {
            htmlToRender(postContainer, element);
        });
        postContainer.insertAdjacentHTML(
