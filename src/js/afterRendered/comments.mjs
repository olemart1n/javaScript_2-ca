...
export const commentList = () => {
    ...
    commentButtons.forEach((btn) => {
        btn.addEventListener(
            "click",
            () => {
                const commentSection = document.getElementById(btn.dataset.commentButton);
                ...
                commentForm.addEventListener("submit", (e) => {
                    ...
                    fetchCall(`posts/${btn.dataset.commentButton}/comment`, entry).then((data) => {
                        console.log(data);
                    });
                    setTimeout(() => {
                        commentForm.reset();
...
const displayComments = (id, relevantComment) => {
    fetchCall(posts + id + "?_author=true&_comments=true&_reactions=true", getWithJwt).then(
        (data) => {
